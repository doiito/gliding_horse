...
fn test_bug3_type_namespace_fix() {
    ...
    .to_string();
    blackboard
        .write_node("iri://task/bug3_test", &node, &config)
        .unwrap();
    ...
    let projection = rt.block_on(async {
        proj.project("iri://task/bug3_test", "summary_only", HashMap::new())
            .await
            .unwrap()
    });
    assert!(
        projection.contains("iri://task/bug3_test"),
        "Bug 3 FAIL: summary_only projection should include the task node. Got: {}",
        &projection[..300.min(projection.len())]
    );
    println!(
        "[Bug 3] summary_only projection includes task node ({} bytes)",
        projection.len()
    );

    // Step 5: pa_init also uses ex:Task — verify it returns artifacts
    let projection_pa = rt.block_on(async {
        proj.project("iri://task/bug3_test", "pa_init", HashMap::new())
            .await
            .unwrap()
    });
    ...
    let projection_da = rt.block_on(async {
        proj.project("iri://task/bug3_test", "da_input", HashMap::new())
            .await
            .unwrap()
    });
    // da_input requires ex:PlanNode, our node is ex:Task, so this should have 0 artifacts
    println!(
        "[Bug 3] da_input projection (expect 0, no PlanNode): {} bytes",
        projection_da.len()
    );
}
...
fn test_bug1_skill_iri_fix() {
    use glidinghorse::skill_graph::types::{Skill5W2H, SkillGraphNode, SkillLinkType};
    ...
    assert!(
        sparql.starts_with("PREFIX skill:"),
    ...
    store.update(&sparql).unwrap_or_else(|e| {
        panic!(
            "Bug 1 FAIL: SPARQL execution failed: {}\nSPARQL:\n---\n{}\n---",
            e, sparql
        )
    });
    ...
    assert!(
        !solutions.is_empty(),
        "Bug 1 FAIL: no CognitiveSkill found (GRAPH <system:test_graph>)"
    );
    println!(
        "[Bug 1] Skill roundtrip confirmed: {} CognitiveSkill node(s)",
        solutions.len()
    );
    ...
    assert_eq!(
        detail_solutions.len(),
        1,
        "Bug 1 FAIL: should find usage+rate data"
    );
...
fn test_bug2_kg_search_entity_type_fix() {
    ...
    println!(
        "[Bug 2] entity_type='TestEntity' (auto-qualified) returned {} result(s)",
        results.len()
    );
    ...
    println!(
        "[Bug 2] Full IRI entity_type also works ({} result(s))",
        results_full.len()
    );
    ...
    println!(
        "[Bug 2] Keyword search without entity_type: {} result(s)",
        results_none.len()
    );
}
...
fn test_bug3_edge_cases() {
    ...
    .to_string();
    blackboard
        .write_node("iri://edge/multi_type", &multi_type, &config)
        .unwrap();
    blackboard.flush_oxigraph();
    ...
    println!(
        "[R2 Bug3-E1] Multi-type node matched via ex:Urgent: {} result(s)",
        results.len()
    );
    ...
    .to_string();
    blackboard
        .write_node("iri://edge/full_iri_type", &full_iri_type, &config)
        .unwrap();
    blackboard.flush_oxigraph();
    ...
    assert_eq!(
        results_full.len(),
        1,
        "Full IRI @type should match directly"
    );
    println!(
        "[R2 Bug3-E2] Full IRI type correctly stored and queryable: {} result(s)",
        results_full.len()
    );
    ...
    for i in 0..10 {
        ...
        .to_string();
        blackboard
            .write_node(&format!("iri://edge/bulk/{}", i), &node, &config)
            .unwrap();
    }
    blackboard.flush_oxigraph();
    ...
    assert!(
        !type_nodes.is_empty(),
        "query_by_types('Task') should return nodes"
    );
    println!(
        "[R2 Bug3-E4] query_by_types('Task') returned {} node(s)",
        type_nodes.len()
    );
    ...
    let projection = rt.block_on(async {
        proj.project("iri://edge", "summary_only", HashMap::new())
            .await
            .unwrap()
    });
    assert!(
        !projection.is_empty(),
        "summary_only projection should return data for bulk nodes"
    );
    println!(
        "[R2 Bug3-E5] summary_only projection across 10 BulkNode nodes OK ({} bytes)",
        projection.len()
    );
}
...
fn test_bug1_edge_cases() {
    use glidinghorse::skill_graph::types::{
        Skill5W2H, SkillGraphMeta, SkillGraphNode, SkillLink, SkillLinkType,
    };
    use oxigraph::sparql::QueryResults;

    // Edge 1: Skill with ALL possible link types
    let mut node = SkillGraphNode::new(
        "iri://skills/edge_complex",
        "edge_complex",
        "Edge case complex skill",
    );
    let all_link_types = [
    ...
    for (link_type, target_iri) in &all_link_types {
        ...
        let sols: Vec<_> = match qr {
            QueryResults::Solutions(s) => s.collect(),
            _ => vec![],
        };
    ...
    let special = SkillGraphNode::new(
        "iri://skills/special_chars",
        "it's \"quoted\" & <encoded>",
    ...
    assert_eq!(
        name_val, "it's \"quoted\" & <encoded>",
        "Name with special chars should roundtrip"
    );
    assert_eq!(
        desc_val, "Description with 'single' and \"double\" quotes",
        "Desc with special chars should roundtrip"
    );
...
fn test_bug2_edge_cases() {
    ...
    println!(
        "[R2 Bug2-E1] Auto-qualified TypeA: {} result(s)",
        results.len()
    );
    ...
    println!(
        "[R2 Bug2-E2] Auto-qualified TypeB: {} result(s)",
        results.len()
    );

    // Edge 3: search with full custom IRI type
    let results = store
        .search_entities("Entity", Some("http://custom.org/types/TypeC"))
        .unwrap();
    assert_eq!(
        results.len(),
        1,
        "search_entities(full IRI TypeC) should find 1"
    );
    println!(
        "[R2 Bug2-E3] Full custom IRI TypeC: {} result(s)",
        results.len()
    );

    // Edge 4: search with non-existent type
    let results = store
        .search_entities("Entity", Some("NonExistentType"))
        .unwrap();
    assert_eq!(
        results.len(),
        0,
        "search_entities('NonExistentType') should find 0"
    );
    ...
    println!(
        "[R2 Bug2-E5] Keyword 'A' + TypeA: {} result(s)",
        results.len()
    );
    ...
    let has_a = results
        .iter()
        .any(|r| r["?s"].as_str().map_or(false, |s| s.contains("type_a")));
...
fn test_all_bugfixes_stress() {
    ...
    for i in 0..100 {
        ...
        .to_string();
        blackboard
            .write_node(&format!("iri://stress/node/{}", i), &node, &config)
            .unwrap();
    }
    blackboard.flush_oxigraph();
    ...
    let even_nodes = blackboard
        .query_by_types(&["EvenType".to_string()])
        .unwrap();
    ...
    println!(
        "[Stress] query_by_types: {} EvenType + {} OddType = 100 total",
        even_nodes.len(),
        odd_nodes.len()
    );
    ...
    let proj_result = rt.block_on(async {
        proj.project("iri://stress", "summary_only", HashMap::new())
            .await
            .unwrap()
    });
    assert!(
        proj_result.len() > 50,
        "summary_only projection should be substantial for 100 nodes"
    );
    println!(
        "[Stress] summary_only projection OK ({} bytes)",
        proj_result.len()
    );
